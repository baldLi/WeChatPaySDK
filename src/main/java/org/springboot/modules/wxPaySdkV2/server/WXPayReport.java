...
public class WXPayReport {
    ...
    public static class ReportInfo {
        ...
        private String sdk = WXPayV2Constants.WXPAYSDK_VERSION;
        ...
        public String toLineString(String key) {
            ...
            try {
                String sign = WXPayV2Util.HMACSHA256(sb.toString(), key);
                sb.append(sign);
                return sb.toString();
            }
            catch (Exception ex) {
    ...
    private WXPayReport(final WXPayV2Config config) {
        this.config = config;
        ...
        if (config.shouldAutoReport()) {
            WXPayV2Util.getLogger().info("report worker num: {}", config.getReportWorkerNum());
                    ...
                    public void run() {
                        while (true) {
                            ...
                            try {
                                ...
                                WXPayV2Util.getLogger().info("get first report msg: {}", firstMsg);
                                String msg = null;
                                sb.append(firstMsg); //会阻塞至有消息
                                int remainNum = config.getReportBatchSize() - 1;
                                for (int j=0; j<remainNum; ++j) {
                                    WXPayV2Util.getLogger().info("try get remain report msg");
                                    // msg = reportMsgQueue.poll();  // 不阻塞了
                                    msg = reportMsgQueue.take();
                                    WXPayV2Util.getLogger().info("get remain report msg: {}", msg);
                                    if (msg == null) {
                                        break;
                                    }
                                    else {
                                ...
                                WXPayV2Util.getLogger().warn("report fail. reason: {}", ex.getMessage());
                            }
                        }
                    }
                });
     ...
     * @return
     */
    public static WXPayReport getInstance(WXPayV2Config config) {
        if (INSTANCE == null) {
    ...
    public void report(String uuid, long elapsedTimeMillis,
        ...
        long currentTimestamp = WXPayV2Util.getCurrentTimestamp();
        ...
        WXPayV2Util.getLogger().info("report {}", data);
        if (data != null) {
    ...
    private static String httpRequest(String data, int connectTimeoutMs, int readTimeoutMs) throws Exception{
        ...
        httpPost.addHeader("User-Agent", WXPayV2Constants.USER_AGENT);
